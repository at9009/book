...
- (void)setMode:(SProgressHUDMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == SProgressHUDModeDeterminate) {
		if ([NSThread isMainThread]) {
			[self updateProgress];
			[self setNeedsDisplay];
		} else {
...
- (void)updateIndicators {
    ...
    else if (mode == SProgressHUDModeCustomView && self.customView != nil){
...
+ (SProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	SProgressHUD *hud = [[SProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
#if __has_feature(objc_arc)
	return hud;
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[SProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		SProgressHUD *HUD = (SProgressHUD *)viewToRemove;
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"SProgressHUDViewIsNillException" 
					format:@"The view used in the SProgressHUD initializer is nil."];
	}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.animationType = SProgressHUDAnimationFade;
        self.mode = SProgressHUDModeIndeterminate;
        self.labelText = nil;
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0f;
    if (animated && animationType == SProgressHUDAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (animationType == SProgressHUDAnimationZoom) {
