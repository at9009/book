...
//  SPhoto
//
//  Created by SunJiangting on 12-8-1.
//  Copyright (c) 2012年 __MyCompanyName__. All rights reserved.
//

#import <UIKit/UIKit.h>

@protocol SProgressHUDDelegate;

/////////////////////////////////////////////////////////////////////////////////////////////

typedef enum {
    /** Progress is shown using an UIActivityIndicatorView. This is the default. */
    SProgressHUDModeIndeterminate,
    /** Progress is shown using a MBRoundProgressView. */
	SProgressHUDModeDeterminate,
	/** Shows a custom view */
	SProgressHUDModeCustomView
} SProgressHUDMode;

typedef enum {
    /** Opacity animation */
    SProgressHUDAnimationFade,
    /** Opacity + scale animation */
    SProgressHUDAnimationZoom
} SProgressHUDAnimation;
 ...
 * The SProgressHUD window spans over the entire space given to it by the initWithFrame constructor and catches all
 ...
 * - SProgressHUDModeIndeterminate - shows a UIActivityIndicatorView
 * - SProgressHUDModeDeterminate - shows a custom round progress indicator (MBRoundProgressView)
 * - SProgressHUDModeCustomView - shows an arbitrary, user specified view (@see customView)
 *
...
@interface SProgressHUD : UIView {
	...
	SProgressHUDMode mode;
    SProgressHUDAnimation animationType;
	...
	id<SProgressHUDDelegate> __weak delegate;
#elif __has_feature(objc_arc)
	id<SProgressHUDDelegate> __unsafe_unretained delegate;
#else
	id<SProgressHUDDelegate> delegate;
#endif
    NSString *labelText;
 ...
 */
+ (SProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated;



+ (SProgressHUD *)showWithView:(UIView *)view status:(NSString *) status animated:(BOOL)animated;
/**
...
/**
 * The UIView (i.g., a UIIMageView) to be shown when the HUD is in SProgressHUDModeCustomView.
...
/** 
 * SProgressHUD operation mode. Switches between indeterminate (SProgressHUDModeIndeterminate) and determinate
 * progress (SProgressHUDModeDeterminate). The default is SProgressHUDModeIndeterminate.
 *
 * @see SProgressHUDMode
 */
@property (assign) SProgressHUDMode mode;

/**
 * The animation type that should be used when the HUD is shown and hidden. 
 *
 * @see SProgressHUDAnimation
 */
@property (assign) SProgressHUDAnimation animationType;

/** 
 * The HUD delegate object. If set the delegate will receive hudWasHidden callbacks when the HUD was hidden. The
 * delegate should conform to the SProgressHUDDelegate protocol and implement the hudWasHidden method. The delegate
 * object will not be retained.
 */
#if __has_feature(objc_arc_weak)
@property (weak) id<SProgressHUDDelegate> delegate;
#elif __has_feature(objc_arc)
@property (unsafe_unretained) id<SProgressHUDDelegate> delegate;
#else
@property (assign) id<SProgressHUDDelegate> delegate;
#endif
...
@protocol SProgressHUDDelegate <NSObject>
 ...
 */
- (void)hudWasHidden:(SProgressHUD *)hud;
